...
import com.haozan.caipiao.activity.topup.UnicomRecharge;
import com.haozan.caipiao.activity.userinf.UserDetailInf;
import com.haozan.caipiao.activity.userinf.UserScoreInf;
 ...
 */
public class UserCenterFragment extends BasicFragment implements
		OnClickListener, OnGetUserInfListener, PopMenuButtonClickListener,
		LoadDataListener, OnHeaderRefreshListener, GetListItemPositionListener,
		OnTabSelectedItemListener, OnGetDataAgainListener {
	...
	private static final String[] TAB_CONTENT = new String[] { "全部", "待开奖",
			"中奖", "追号", "合买" };
	...
	private String[] titlePopupTextName = { "消息", "充值记录", "财园资料", "个人信息",
			"中奖通知", "反馈" };
	private String[] topBarMenuClassName = { "userinf.MessageCenter",
			"userinf.TopupHistory", "weibo.MyProfileActivity",
			"UserInf_ModifyPassword", "AdditionalService", "Feedback" };
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		mLayout = View.inflate(mContext, R.layout.activity_user_center, null);
		setupViews(mLayout);
		return mLayout;
	}
	...
	private void setupViews(View layout) {
		setupMainViews(layout);
		setupHeaderViews();
	}

	private void setupMainViews(View layout) {
		mPullToRefreshView = (RefreshLayout) layout.findViewById(R.id.user_new_center_pull_refresh_view);
		mPullToRefreshView.setOnHeaderRefreshListener(this);

		autoListView = (AutoLoadListView) layout.findViewById(R.id.bet_history_list);
		autoListView.setOnLoadDataListener(this);
		autoListView.setOnGetListItemPositionListener(this);
		warningInfLayou = (RelativeLayout) layout.findViewById(R.id.warning_inf_layout);
		...
		iBTopMenu = (ImageButton) layout.findViewById(R.id.top_menu);
		iBTopMenu.setOnClickListener(this);

		layoutsEmpty = (EmptyLayout) layout.findViewById(R.id.show_fail_page);
		layoutsEmpty.setOnGetDataAgainListener(this);
	}

	private void setupHeaderViews() {
		View headView = View.inflate(mParentActivity,
				R.layout.user_new_center_list_header, null);

		layoutScore = (LinearLayout) headView
				.findViewById(R.id.user_new_center_score_detail);
		layoutScore.setOnClickListener(this);
		layoutBalance = (LinearLayout) headView
				.findViewById(R.id.user_new_center_account_detail);
		layoutBalance.setOnClickListener(this);
		tvScore = (TextView) headView
				.findViewById(R.id.user_new_center_header_points);
		tvBalance = (TextView) headView
				.findViewById(R.id.user_new_center_header_balances);
		...
		tvWithdraw.setText(Html
				.fromHtml("<u><font color='#FFFFFF'>奖金提现></font></u>"));

		topMenuBarLayout = (TopMenuLayout) headView
				.findViewById(R.id.top_menu_bar_layout);
	...
	private void checkPerfectInf() {
		...
		if ("0".equals(appState.getPerfectInf())
				|| !"1".equals(appState.getRegisterType())
	...
	private void init() {
		...
		userNewCenterAdapter = new UserNewCenterAdapter(mParentActivity,
				betHistoryList);
	...
	private void showPopView() {
		titlePopup = new PopMenu(mParentActivity, false);
		// 设定类型 如果类型是unc就用列表的形式显示。
		titlePopup.setLotteryType("unc");
		titlePopup.setLayout(R.layout.pop_list_view, titlePopupTextName, null,
				3, mLayout.findViewById(R.id.title_layout).getWidth() - 150,
				-1, false, false);
		titlePopup.setButtonClickListener(this);
		titlePopup.showAtLocation(mPullToRefreshView, Gravity.RIGHT
				| Gravity.TOP, 10, mLayout.findViewById(R.id.title_layout)
				.getHeight() + 40);
	}
	...
	private void showFail(String failInf) {
		if (betHistoryList.size() == 0) {
			...
			ViewUtil.showTipsToast(mParentActivity, failInf);
	...
	private void showNoData() {
		if (betHistoryList.size() == 0) {
			...
			ViewUtil.showTipsToast(mParentActivity, "现只能查询最近3个月的数据");
		}
	}
	...
	public void onClick(View view) {
		if (view.getId() == R.id.caijin_title_tv) {
			...
			intent.setClass(mParentActivity, CaiJinList.class);
			startActivity(intent);
		} else if (view.getId() == R.id.top_menu) {
			showPopView();
		} else if (view.getId() == R.id.top_up) {
			ActionUtil.toTopupNew(mParentActivity);
			//Intent intent = new Intent();
			//intent.setClass(mParentActivity, UnicomRecharge.class);
			//startActivity(intent);
		} else if (view.getId() == R.id.user_new_center_score_detail) {
			Intent intent = new Intent();
			intent.setClass(mParentActivity, UserScoreInf.class);
			startActivity(intent);
		} else if (view.getId() == R.id.user_new_center_account_detail) {
			Intent intent = new Intent();
			intent.setClass(mParentActivity, UserDetailInf.class);
			startActivity(intent);
			...
			intent.setClass(mParentActivity, PerfectInf.class);
			...
			if ("0".equals(appState.getPerfectInf())
				...
				if (appState.getBand() == 0) {
					intent.setClass(mParentActivity, UserWithdraw.class);
				} else if (appState.getBand() == 1) {
					intent.setClass(mParentActivity, UserWithdrawExtra.class);
				}
				startActivity(intent);
			}
		}
	...
	public void onPost(String result) {
		if (result == null) {
			ViewUtil.showTipsToast(mParentActivity,
			...
			if (status.equals("200")) {
				...
				OperateInfUtils.clearSessionId(mParentActivity);
				showLoginAgainDialog(getResources().getString(
	...
	class BetHistoryTask extends AsyncTask<Integer, Object, String> {
		...
		private void userBetHistoryResponseData(String data) {
			try {
				...
				for (int i = 0; i < hallArray.length(); i++) {
					...
					String month = StringUtil.deleteZeroPrefix(TimeUtils
							.convertDate(date, "yyyy-MM-dd HH:mm:ss", "MM"));
					String day = TimeUtils.convertDate(date,
							"yyyy-MM-dd HH:mm:ss", "dd");
					item.setMonth(month + "月");
					item.setDay(day);
					item.setDayOfWeek(TimeUtils.dayOfWeek(TimeUtils
							.stringConvertToDate(date)));
					if (i == 0 && oldLength == 0) {
						item.setShowDate(true);
					} else if (i == hallArray.length()) {
						item.setShowFinished(true);
					} else {
						String lastMonth = betHistoryList
								.get(oldLength + i - 1).getMonth();
						String lastDay = betHistoryList.get(oldLength + i - 1)
								.getDay();
						if (lastMonth.equals(month) && lastDay.equals(day)) {
							item.setShowDate(false);
							betHistoryList.get(oldLength + i - 1)
					...
					setOrderInf(item, lotteryOrderStatus, item.getType(),
							lotteryOrderWinStatus, prize);
		...
		private void setOrderInf(UserCenterBetHistoryItem item,
				int lotteryOrderStatus, int lotteryOrdertype,
			...
			if (listDataType == LotteryUtils.AlL_lOTTERY_DATA
				...
				if (((lotteryOrderStatus == 0 && lotteryOrdertype != 2) || (lotteryOrderStatus != 0 && lotteryOrdertype == 2))
					...
					item.setAward(true);
				} else if (lotteryOrderStatus == 0 && lotteryOrdertype == 2
						&& lotteryOrderWinStatus == 1) {
					item.setStatusColor(getResources().getColor(R.color.red));
					item.setStatus("进行中,中" + prize + "元");
					item.setTypeDescription(getOrderType(lotteryOrdertype));
					item.setAward(true);
				} else if (lotteryOrdertype != 5 && lotteryOrderStatus == 0
						&& lotteryOrderWinStatus == 0) {
					item.setStatusColor(getResources().getColor(
					...
					item.setStatus(getBetStatus(lotteryOrdertype,
							lotteryOrderStatus));
					item.setTypeDescription(getOrderType(lotteryOrdertype));
					item.setAward(false);
				}
	...
	@Override
	public void setPopMenuButtonClickListener(String dateStart, String dateEnd,
			String dayDuration, int kind, String tabName) {
		titlePopup.dismiss();
		String packName = LotteryUtils.getPackageName(mParentActivity)
				+ ".activity";
		...
		if (tabName.contains("中奖通知")) {
			if ("1".equals(registerType))
				SMSNoticeService(intent, c);
					...
					&& !"0".equals(appState.getPerfectInf()))
				SMSNoticeService(intent, c);
			...
			intent.setClass(mParentActivity, c);
	...
	private void getUserInf() {
		if (HttpConnectUtil.isNetworkAvailable(mParentActivity)) {
			UserInfTask getUserInf = new UserInfTask(mParentActivity);
	 ...
	 */
	class GetMessageTask extends AsyncTask<Void, Object, String> {
		...
		protected void onPostExecute(String json) {
			...
			if (json != null) {
				...
				if (status.equals("200")) {
					...
					OperateInfUtils.broadcast(mParentActivity,
							"invalidate_notice");
				}
			}
		}
	...
	@Override
	public void processListItemPosition(AbsListView view, int firstVisibleItem,
		...
		if (listView.getChildAt(firstVisibleItem) != null) {
			...
			if (firstVisibleItem == 0) {
				listChildViewTop = listView.getChildAt(0).getHeight()
