...
public class AsyncImageLoader {
    ...
    public void loadPortrait(final long id, final String url, final ImageView iv) {
        ...
        executorService.submit(new Runnable() {
            ...
            @Override
            public void run() {
                bitmapDrawable = localMemory.getDrawable(String.valueOf(id), LocalMemory.PORTRAIT);
                if (bitmapDrawable == null) { // 若本地没有指定图片，从网上下载
                    try {
                        bitmapDrawable = new BitmapDrawable(new URL(url).openStream()); // 获取图片
                        imageCache.put(id, new SoftReference<Drawable>(bitmapDrawable)); // 加入软引用
                        AsyncImageSaver.getInstance().saveImage(bitmapDrawable, String.valueOf(id),
                                                                LocalMemory.PORTRAIT); // 异步保存至本地
                    }
    ...
    public void loadPre(final long id, final String url, final ImageView iv) {
        ...
        executorService.submit(new Runnable() {
            ...
            @Override
            public void run() {
                bitmapDrawable = localMemory.getDrawable(TextUtil.formatName(url), LocalMemory.PRE);
                if (bitmapDrawable == null) { // 若本地没有指定图片，从网上下载
                    try {
                        bitmapDrawable = new BitmapDrawable(new URL(url).openStream()); // 获取图片
                        imageCache.put(id, new SoftReference<Drawable>(bitmapDrawable)); // 加入软引用
                        AsyncImageSaver.getInstance().saveImage(bitmapDrawable, TextUtil.formatName(url),
