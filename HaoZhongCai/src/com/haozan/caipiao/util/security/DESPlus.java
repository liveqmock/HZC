...
public class DESPlus {
    ...
    private final static String ENCRYPT_KEY = "BukeInfo2012IljiLJ";
     ...
     */
    private Key getKey(byte[] data)
        throws Exception {
        // 创建一个空的8位字节数组（默认值为0）
        byte[] arrB = new byte[8];

        // 将原始字节数组转换为8位
        for (int i = 0; i < data.length && i < arrB.length; i++) {
            arrB[i] = data[i];
        }
    ...
    public static String deCode(String password) {
        try {
            password = new DESPlus(DESPlus.getEncryptKey()).decrypt(password);
        }
        catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public static String enCode(String password) {
        try {
            password = new DESPlus(DESPlus.getEncryptKey()).encrypt(password);
        }
        catch (Exception e) {
            e.printStackTrace();
        }
