...
public class BetPayUniteSports
    extends BetPayUnite
    implements OnTaskChangeListener, OnFocusChangeListener {
    private static final String BET_PAY_UNITE_SPORTS_REQUEST_SERVICE = "2003071";
    ...
    private void initSpecail() {
        ...
        bunchAdapter = new SportsBunchAdapter(BetPayUniteSports.this, bunchInfList);
    ...
    private void excuteZuicaiTask() {
        if (HttpConnectUtil.isNetworkAvailable(BetPayUniteSports.this)) {

            if (kind.equals("sfc") || kind.equals("r9")) {
                SportLotteryInfTask sportLotteryInfTask =
                    new SportLotteryInfTask(BetPayUniteSports.this, matchDataList, kind, term);
    ...
    private void displayBetSFCCode() {
        ...
        drawBalls.drawSFCHistoryBall(BetPayUniteSports.this, zucaiPredicate, kind, betCode, teams,
                                     screenWidth);
    }
    ...
    private void setupSpecailView() {
        ...
        tv_buynum_ll.setOnClickListener(this);
        tv_buynum_ll.setOnFocusChangeListener(this);
        tv_guanum_ll.setOnClickListener(this);
        tv_guanum_ll.setOnFocusChangeListener(this);
        editShare.setOnFocusChangeListener(this);
    }

    protected void refreshBunchInf() {
        // bunchInfStr 过关方式显示
    ...
    private void caluculateBetMoney() {
        ...
        betCodeFinal.append(":");
        money = sum * 2;
        lastNums = (int) (money / unitePrice);// 总份数，根据设置的每份金额进行改变
...
// }
    ...
    protected void toBet() {
        if (wholeMoney < 8) {
            ViewUtil.showTipsToast(BetPayUniteSports.this, PROJECT_MIN_MONEY);
        }
        else {
    ...
    public class BetTask
        ...
        protected String doInBackground(String... kind) {
            ConnectService connectNet = new ConnectService(BetPayUniteSports.this);
            String json = null;
            try {
        ...
        protected void onPostExecute(String json) {
            ...
            progress.dismiss();
            if (json != null) {
                Logger.inf("vincent", "竞彩足球合买返回：" + json);
                String inf = null;
                JsonAnalyse ja = new JsonAnalyse();
                // get the status of the http data
                String status = ja.getStatus(json);
                if (status.equals("200")) {
                    mUploadRequestTime.submitConnectSuccess(BET_PAY_UNITE_SPORTS_REQUEST_SERVICE);
                    ...
                    intent.setClass(BetPayUniteSports.this, BetSuccessPage.class);
                    ...
                    OperateInfUtils.clearSessionId(BetPayUniteSports.this);
                ...
                Logger.inf("vincent", "竞彩足球合买返回：" + "json = null");
                error = "网络不稳定，请查看个人中心是否投注成功，如需帮助请联系客服";
            }
            if (error != null) {
                mUploadRequestTime.submitConnectFail(BET_PAY_UNITE_SPORTS_REQUEST_SERVICE, json);

                ViewUtil.showFailDialog(BetPayUniteSports.this, error);
            }
        }
    ...
    public HashMap<String, String> initHashMap() {
        ...
        parameter.put("service", BET_PAY_UNITE_SPORTS_REQUEST_SERVICE);// 服务接口名称
        parameter.put("pid", LotteryUtils.getPid(BetPayUniteSports.this));// 分配的ID号
...
// parameter.put("todayluck", HttpConnectUtil.encodeParameter(todayLucky));
        ...
        parameter.put("source",
                      HttpConnectUtil.encodeParameter(LotteryUtils.versionName(BetPayUniteSports.this, true)));
        if (betInfTranspond)
    ...
    public void onClick(View v) {
        ...
        else if (v.getId() == R.id.tv_buynum_ll || v.getId() == R.id.tv_guanum_ll ||
            v.getId() == R.id.edit_share) {
            if (showBunch) {
                bunchGridView.setVisibility(View.GONE);
                showBunch = false;
                statusIcon.setImageResource(R.drawable.icon_up_new);
    ...
    public void onFocusChange(View v, boolean hasFocus) {
        if (v.getId() == R.id.tv_buynum_ll || v.getId() == R.id.tv_guanum_ll || v.getId() == R.id.edit_share) {
            if (hasFocus) {
                if (showBunch) {
                    bunchGridView.setVisibility(View.GONE);
                    showBunch = false;
