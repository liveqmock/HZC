 ...
 */
public class UnionPayDebitSelectBankControl
    extends SelectBankControlBasic {

    public UnionPayDebitSelectBankControl(Context context, Handler handler) {
        super(context, handler);
    }

    public void insertBankFromDb(ArrayList<BankInfo> bankList) {
        Cursor cursor =
            mContext.getContentResolver().query(BankTable.CONTENT_URI, null, BankDbBean.DEPOSIT + "!=''",
    ...
    public void selectedBank(BankInfo bank) {
        ...
        bundle.putInt("type", TopupRecommend.UNIONPAY_DEBIT);
        bundle.putString("way", bank.getDeposit());
